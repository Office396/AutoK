    ...
    def _parse_row(self, row) -> Optional[SiteInfo]:
        ...
        try:
            ...
            return SiteInfo(
                ...
                share_holder=get_val(13) if len(values) > 13 else '',
                remarks=get_val(14) if len(values) > 14 else '',
                site_status=get_val(19) if len(values) > 19 else '',
                omo_b2s_name=get_val(20) if len(values) > 20 else '',
                omo_b2s_id=get_val(21) if len(values) > 21 else '',
                hw_mbu_lead=get_val(22) if len(values) > 22 else '',
                day_tech=get_val(23) if len(values) > 23 else '',
                night_tech=get_val(24) if len(values) > 24 else '',
                jazz_mbu_tech=get_val(25) if len(values) > 25 else '',
                jazz_mbu_lead=get_val(26) if len(values) > 26 else '',
                dependency_count=get_val(27) if len(values) > 27 else '',
                connectivity=get_val(28) if len(values) > 28 else '',
                ftts_ring_id=get_val(29) if len(values) > 29 else '',
                site_type_new=get_val(30) if len(values) > 30 else '',
                dependent=get_val(31) if len(values) > 31 else '',
                new_dependent=get_val(32) if len(values) > 32 else ''
            )
        except Exception as e:
            return None
